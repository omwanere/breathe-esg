...
from esg_platform.models import IngestionJob, RawActivityRow, DataSource, UnitConversion, EmissionFactor, AuditLog
...
def parse_german_decimal(val):
    ...
    if not val_str:
        return Decimal('0')
    # If the value contains dot and comma, or ends with comma, it is likely German format
    # E.g. "1.234,56" -> remove dots, replace comma with dot
    # If it's just "1234,56" -> replace comma with dot
    if ',' in val_str:
...
def ingest_sap_file(job_id, file_obj, filename):
    ...
    try:
        ...
        for index, row in enumerate(data_generator, start=1):
            ...
            # Filter movement types (261, 201, 101)
            ...
            # 4. Material classification for scope and source_type
            material = str(row.get('Material', '')).strip()
            desc = str(row.get('Materialkurztext', '')).strip()
            
            # Logic: Diesel or petrol/Benzin is scope 1 (fuel), others are procurement (scope 3)
        ...
        # Log audit trail for all created rows
        audit_logs = []
        ...
        # Update data source last ingested
        ds = job.data_source
    ...
    try:
        ...
        for index, row in enumerate(data_generator, start=1):
            ...
            if expense_type.lower() == 'airfare':
                ...
                # Calculate from IATA codes if distance is 0 or empty
                ...
                # Ground Travel (Car Rental, Rail, Taxi/Rideshare)
                source_type = 'TRAVEL_GROUND'
                mapped_unit = 'km'
                dist_raw = row.get('distance_km')
                try:
    ...
    try:
        if source_type in ('SAP_FUEL', 'SAP_PROCUREMENT'):
            ...
            if is_diesel:
                ...
                # SAP Procurement or other material - no predefined factor, require manual factor or set to 0/flagged
                # Let's see if we have a matching factor by description or name
                ...
                if ef:
                    ...
                    # Allow approving but set co2e to 0
            ...
            # booking_class Y=1.0, W=1.6, C=2.0, F=2.4
            ...
            # Flight distance threshold: short-haul < 3700km, long-haul >= 3700km (standard DEFRA breakdown)
            ...
            # Ground travel (Taxi, Car Rental, Rail)
            # Find expense type
